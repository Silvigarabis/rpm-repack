...
if [ ! -r $MOZ_LIB_DIR/firefox-dev/$MOZ_FIREFOX_FILE ]; then
    if [ ! -r $SECONDARY_LIB_DIR/firefox-dev/$MOZ_FIREFOX_FILE ]; then
	echo "Error: $MOZ_LIB_DIR/firefox-dev/$MOZ_FIREFOX_FILE not found"
	if [ -d $SECONDARY_LIB_DIR ]; then
	    echo "       $SECONDARY_LIB_DIR/firefox-dev/$MOZ_FIREFOX_FILE not found"
	fi
	exit 1
    fi
    MOZ_LIB_DIR="$SECONDARY_LIB_DIR"
fi
MOZ_DIST_BIN="$MOZ_LIB_DIR/firefox-dev"
...
##
export MOZ_APP_LAUNCHER="/usr/bin/firefox-dev"
...
then
  export MOZ_APP_REMOTINGNAME=org.mozilla.firefox-dev
fi
